...
class sveawebpay_creditcard {
  ...
  function update_status() {
    ...
    if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_SWPCREDITCARD_ZONE > 0) ) {
      $check_flag = false;
      $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_SWPCREDITCARD_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
  ...
  function process_button() {
    ...
    $configSvea->merchantId = MODULE_PAYMENT_SWPCREDITCARD_MERCHANT_ID;
    $configSvea->secret = MODULE_PAYMENT_SWPCREDITCARD_SW; 
    ...
    $orderSvea->paymentMethod = SveaOrder::CARD;
  ...
  function install() {
    ...
    tep_db_query($common . ", set_function) values ('Enable SveaWebPay Card Module', 'MODULE_PAYMENT_SWPCREDITCARD_STATUS', 'True', 'Do you want to accept SveaWebPay payments?', '6', '0', now(), 'tep_cfg_select_option(array(\'True\', \'False\'), ')");
    tep_db_query($common . ") values ('SveaWebPay Merchant ID', 'MODULE_PAYMENT_SWPCREDITCARD_MERCHANT_ID', '', 'The merchant ID obtained from Svea', '6', '0', now())");
    tep_db_query($common . ") values ('SveaWebPay Secret word', 'MODULE_PAYMENT_SWPCREDITCARD_SW', '', 'The secret word used for hosted payment', '6', '0', now())");
    ...
    tep_db_query($common . ", set_function) values ('Default Currency', 'MODULE_PAYMENT_SWPCREDITCARD_DEFAULT_CURRENCY', 'SEK', 'Default currency used, if the customer uses an unsupported currency it will be converted to this. This should also be in the supported currencies list.', '6', '0', now(), 'tep_cfg_select_option(array(\'SEK\',\'NOK\',\'DKK\',\'EUR\'), ')");
    ...
    tep_db_query($common . ", set_function, use_function) values ('Payment Zone', 'MODULE_PAYMENT_SWPCREDITCARD_ZONE', '0', 'If a zone is selected, only enable this payment method for that zone.', '6', '2', now(), 'tep_cfg_pull_down_zone_classes(', 'tep_get_zone_class_title')");
    tep_db_query($common . ") values ('Sort order of display.', 'MODULE_PAYMENT_SWPCREDITCARD_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '0', now())");
  }
